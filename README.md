...
- アプリの残高欄は接続アドレスの**オンチェーンJPYC残高**です。預入・運用型ウォレットではサービス内残高と異なる場合があります（[表示範囲とUnifiに関する注意](./docs/WALLET_BALANCE_LIMITATIONS.md)）
...
本プロジェクトは [MIT License](./LICENSE) で公開しています。個人利用・研究・商用利用を問わず、MIT Licenseの条件に従って利用、改変、再配布できます。商用利用に開発者の事前許可は必要ありません。

導入支援、カスタマイズ、運用サポートはライセンスとは別の有償サービスとして提供できます。本番環境へ導入する場合は、利用者の責任でセキュリティ、法令対応、バックアップ、スマートコントラクトおよび決済フローを十分に検証してください。
