---
title: "【失敗から学ぶ】SBTスタンプカード×JPYC決済アプリ開発の全記録"
emoji: "🎫"
...
ブロックチェーン技術を活用したSBT（Soulbound Token）スタンプカードとJPYC決済を統合したPWAアプリを開発しました。

**JPYCとは？**  
JPYC（JPY Coin）は**日本円と1:1で連動するステーブルコイン**です。1 JPYC = 1円の価値を持ち、価格変動リスクがないため、**暗号資産というより電子マネーに近い決済手段**として利用できます。
...
- **QRコード決済**: JPYC（日本円ステーブルコイン）による決済用QRコード生成
- **マルチチェーン対応**: Polygon Mainnet / Amoy Testnet
- **PWA対応**: スマートフォンでネイティブアプリのように動作

:::message
💴 **JPYCの特徴**  
- **価格安定**: 1 JPYC = 1円で固定（価格変動なし）
- **決済手段**: ビットコインのような投機対象ではなく、日常決済に特化
- **低コスト**: Polygonネットワーク利用で送金手数料が数円程度
- **即時送金**: 銀行振込と違い、24時間365日即座に送金可能
:::
...
JPYCが店舗に普及することで、**価格変動のないブロックチェーン決済**が広まり、キャッシュレス決済のさらなる普及に寄与できると信じています。

従来の暗号資産（ビットコインやイーサリアム）は価格変動が激しく、店舗側が受け取った瞬間に価値が下がるリスクがありました。しかし、**JPYCは日本円と1:1で連動**しているため、そのリスクがありません。

```
例: 100円の商品を販売
❌ ビットコイン: 受取時 0.001 BTC → 翌日 95円相当（5%損失リスク）
✅ JPYC: 受取時 100 JPYC → 常に100円（価値固定）
```

特に、個人経営の小規模店舗や地域密着型の商店でも導入しやすい仕組みを目指しました。
